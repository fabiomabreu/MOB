...
public class CriptografiaUrlHelper : Dictionary<string, string>
{
	protected byte[] _keyBytes = new byte[8] { 17, 18, 19, 20, 21, 22, 23, 24 };

	protected string _keyString = "TGT12345";

	protected string _checksumKey = "__$$";

	public CriptografiaUrlHelper()
	{
	}

	public CriptografiaUrlHelper(string encryptedData)
	{
	...
	protected string ComputeChecksum()
	{
		...
		using (Enumerator enumerator = GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				num += current.Key.Sum((char c) => c - 48);
				num += current.Value.Sum((char c) => c - 48);
			}
		}
