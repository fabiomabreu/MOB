...
internal class ExportCadastroLinha
{
	...
	public ExportCadastroLinha(string stringConnTargetErp, string cnpjEmpresa, BasicHttpBinding bindingBasicHttp, EndpointAddress remoteAddress)
	{
	...
	public void Exportar(object eventosAtivos)
	{
		...
		try
		{
			...
			byte[] rowId = wsErpSoapClient.WsERP_RowId_GetV2(validationSoapHeader, _CnpjEmpresa, EnumModel.Linha, Seguranca.getHostName());
			dbConnection.Open();
			LinhaTO[] array = LinhaBLL.Select(dbConnection, null, null, null, null, null, rowId);
			if (array != null)
			{
				List<LinhaWsModel> list = new List<LinhaWsModel>();
				LinhaTO[] array2 = array;
				foreach (LinhaTO linhaTO in array2)
				{
					LinhaWsModel linhaWsModel = new LinhaWsModel();
					linhaWsModel.Ativo = linhaTO.Ativo;
					linhaWsModel.CdLinha = linhaTO.CdLinha;
					linhaWsModel.Descricao = linhaTO.Descricao;
					linhaWsModel.EnvioPalmTop = linhaTO.EnvioPalmTop;
					linhaWsModel.CdCategoriaProduto = linhaTO.CdCategprd;
					linhaWsModel.RowId = linhaTO.RowId;
					list.Add(linhaWsModel);
				}
				RetornoWsModelOfBoolean retornoWsModelOfBoolean = wsErpSoapClient.WsERP_Linha_Setar(validationSoapHeader, _CnpjEmpresa, list.ToArray(), Seguranca.getHostName());
