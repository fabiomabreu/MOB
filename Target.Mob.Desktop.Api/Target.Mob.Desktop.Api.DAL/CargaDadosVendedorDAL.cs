...
public class CargaDadosVendedorDAL
{
	private const string SELECT_DADOS_VENDEDOR = "tgtmob_uspCargaDadosVendedor";

	private const string SELECT_IS_CARGA_COMPLETA = "tgtmob_uspCargaDadosVendedor_IsCargaCompleta";

	public static IEnumerable<DadosTO> SelectDadosVendedor(string stringConnTargetErp, string codigoVendedor, long ultimoIdCargaEntidade, long cacheId)
	{
		int num = 20000;
	...
	internal static bool IsCargaCompleta(string conn, string codigoVendedor)
	{
		bool result = false;
		using SqlConnection sqlConnection = new SqlConnection(conn);
		sqlConnection.Open();
		using (SqlCommand sqlCommand = new SqlCommand("tgtmob_uspCargaDadosVendedor_IsCargaCompleta", sqlConnection))
		{
			sqlCommand.CommandType = CommandType.StoredProcedure;
			sqlCommand.Parameters.Clear();
			sqlCommand.Parameters.AddWithValue("@CodigoVendedor", codigoVendedor);
